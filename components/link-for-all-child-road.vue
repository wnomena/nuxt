<template>
    <section class="row mt-3 mb-3">
    <div class="col-lg-10 col-md-12 border d-flex justify-content-center m-auto rounded">
        <div class="col-12 text-center">
            <h3>Travel differently</h3>
            <p class="fs-5">Visit diversity that is not found anywhere else in the world, whether it is fauna or flora</p>
        <div class="col-12 d-flex justify-content-center mb-2">
            <div class="border col-8 text-center bg-success rounded bg-opacity-25-hover"><NuxtLink class="col-12 text-light text-decoration-none" to="/">All our circuits bring together.</NuxtLink></div>
        </div>
        </div>
    </div>
</section>
</template>

<style scoped>
    .row .col-lg-8 {
    box-shadow: inset 0 0 2px 1px green;
    margin-top: 10px;
}
.row .col-lg-8 .col-12 .col-12 .border {
    color: lightgoldenrodyellow;
    font-weight: 500;
    box-shadow: none;
    background-color: green;
}
</style>